...
def test_workflow_authority_tokens_are_memory_only_and_required_for_push():
    ...
    assert "state.pushExecutionGuardSatisfiedToken?.workflowId !== state.workflow.id" in text
    assert "The workflow is in push, but this pi session has no in-memory push execution guard satisfied token" in text
...
def test_manual_state_and_abort_have_explicit_yes_no_confirm_copy():
    ...
    assert "예: 위 내용을 내가 확인하고 phase/token을 복구합니다." in text
    assert "아니오: phase와 token을 변경하지 않습니다." in text
    assert "state.dpaaGuardSatisfiedToken = null" in text
    assert "state.pushExecutionGuardSatisfiedToken = null" in text
    assert "Code quality guard satisfied → code quality guard satisfied token 발급" in text
    assert "Code review guard satisfied → code review guard satisfied token 발급" in text
