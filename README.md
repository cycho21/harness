...
요구사항 수집은 workflow의 기본 `interview → plan` 흐름에 집중합니다. 별도 feature-interview/planning-room/requirements-room skill과 slash command는 제거했습니다. `interview` skill은 deep-interview-lite 방식으로 동작해, LLM이 제공한 5개 baseline 질문(범위, 동기, 완료 기준, 영향 파일/모듈, 제약/위험)을 그대로 묻습니다. Runtime은 work map/topology 질문이나 clarity checkpoint를 자동 추가하지 않습니다. 필요하면 LLM이 baseline 질문 안에 topology 또는 clarity 질문을 직접 포함합니다. Choice는 선택 필드 `recommended`와 `recommendationReason`을 가질 수 있으며, 추천 표시는 advisory 정보입니다. 사용자가 선택한 답변이 source of truth입니다. Brownfield 작업에서는 좁은 repo evidence를 먼저 인용하며, 목표/범위/완료 기준/제약/기존 맥락 중 가장 약한 명확성 차원을 기준으로 후속 질문합니다. 낮은 점수 차원을 다시 묻는 follow-up wizard는 `round: 'follow_up'`으로 실행해 targeted 질문만 표시합니다. Wizard 완료 후 LLM은 `workflow_score_interview`로 목표/범위/완료 기준/제약/기존 맥락 점수(각 0–100)를 기록해야 하며, 한 차원이라도 60 미만이면 `interview → plan` 전이가 차단됩니다.
...
EPIC PEV Queue는 큰 인터뷰에서 여러 작업이 나온 경우에만 사용하는 선택적 task queue입니다. 기존 workflow phase는 그대로 유지하고 `WorkflowInstance.taskQueue`의 active task만 전환합니다. `workflow_task_queue` tool은 `create`, `add`, `activate`, `mark`, `show` operation으로 queue를 만들고 하나의 active task를 선택하며, 상태는 `pending`, `active`, `done`, `blocked`, `deferred` 중 하나로 기록됩니다. 전체 queue snapshot은 `.ai/interview/runs/<workflowId>/task-queue.json`에 저장되고, prompt context에는 queue 제목, active task, 상태 count, 다음 pending task, active task의 scope/acceptance/verification 단서만 bounded summary로 요약됩니다. implement 단계의 autonomous task execution policy는 이 active task를 goal lock으로 삼아 next action, test decision, failure escape, completion/switch 판단을 자율적으로 수행하도록 안내합니다. 이 정책은 새 hard gate가 아니라 prompt-contract/advisory 규칙입니다. 이 기능은 무제한 discovery가 아니며, 범위 밖 발견은 자동 구현하지 않고 pending/deferred task로 기록합니다. 기존 단일 workflow 사용과 `commit → push` 승인 경계는 변경되지 않습니다.
...
`/workflow start <목표>`가 `interview` phase를 시작하면 UI 세션에서는 interview wizard가 자동으로 열립니다. LLM은 기존 5개 baseline 질문을 제공하고, runtime은 해당 질문만 표시합니다. Runtime은 work-map/topology 질문이나 clarity checkpoint를 자동 추가하지 않으며, 범위와 영향 파일/모듈 coverage는 baseline 질문 답변에서 확보합니다. 필요하면 LLM이 baseline 질문 안에 topology 또는 clarity 질문을 직접 포함합니다. 선택지는 optional `recommended`와 `recommendationReason`을 포함할 수 있고, TUI는 추천 선택지에 `[추천]` badge와 이유 줄을 표시합니다. 추천 표시는 자동 선택이 아니며 사용자가 고른 답변이 source of truth입니다. 낮은 clarity dimension을 다시 묻는 follow-up wizard는 `round: 'follow_up'`으로 실행해 targeted 질문만 표시합니다. Wizard는 질문을 하나씩 보여주며, 대부분의 질문에서 선택지와 자유입력을 함께 받고 선택 질문은 `모름/건너뛰기`를 허용합니다. 필수 질문은 선택지 또는 자유입력 없이 다음으로 진행할 수 없습니다. Editor 위 progress widget은 현재/완료/남은 질문을 표시하고, footer status는 workflow title, 현재 phase, 다음 phase, 전체 phase progress를 표시합니다. Wizard 중 preview 키로 답변 요약을 볼 수 있으며 마지막 질문 후 같은 요약 preview가 자동 표시됩니다. Wizard 결과는 LLM에게 baseline 범위 및 영향 파일/모듈 답변을 spec/plan 필수 coverage로 취급하고, brownfield follow-up 전 좁은 repo evidence를 확인하며, 가장 약한 clarity dimension을 대상으로 추가 질문하라고 안내합니다. UI가 없거나 wizard가 취소/실패하면 기존 채팅 기반 interview continuation으로 fallback됩니다.
