...
EPIC PEV Queue는 큰 인터뷰에서 여러 작업이 나온 경우에만 사용하는 선택적 task queue입니다. 기존 workflow phase는 그대로 유지하고 `WorkflowInstance.taskQueue`의 active task만 전환합니다. `workflow_task_queue` tool은 `create`, `add`, `activate`, `mark`, `show` operation으로 queue를 만들고 하나의 active task를 선택하며, 상태는 `pending`, `active`, `done`, `blocked`, `deferred` 중 하나로 기록됩니다. 전체 queue snapshot은 `.ai/interview/runs/<workflowId>/task-queue.json`에 저장되고, prompt context에는 queue 제목, active task, 상태 count, 다음 pending task만 요약됩니다. 이 기능은 무제한 discovery가 아니며, 범위 밖 발견은 자동 구현하지 않고 pending/deferred task로 기록합니다. 기존 단일 workflow 사용과 `commit → push` 승인 경계는 변경되지 않습니다.
