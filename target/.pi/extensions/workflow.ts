...
function buildDeepInterviewLiteQuestions(_workflowTitle: string, baselineQuestions: InterviewQuestion[], _round: "initial" | "follow_up" = "initial"): InterviewQuestion[] {
  return baselineQuestions.map((question) => normalizeInterviewQuestion(question));
}
...
function buildInterviewWizardCompletion(summaryMarkdown: string): string {
  return [
    ...
    "1. Treat the baseline scope and affected-files/modules answers as required coverage for the spec and plan.",
...
${formatWorkflowAction(state.workflow)}` }],
  ...
  pi.registerTool({
    ...
    description: [
      ...
      "For every round, the tool displays exactly the questions provided by the LLM and adds no automatic scaffold questions. For follow-up rounds, pass round: 'follow_up' to document that the provided questions are targeted follow-ups.",
    ...
    promptGuidelines: [
      "Call workflow_interview_wizard immediately when an interview phase starts. Generate 5 baseline questions (scope, motivation, acceptance criteria, affected files/modules, constraints/risks) with domain-specific choices for the workflow goal. Choices may include optional recommended and recommendationReason fields to mark advisory recommendations; user selection remains the source of truth. The tool adds no automatic scaffold questions; if topology or clarity questions are useful, include them directly in the provided baseline. For low-score follow-ups, pass round: 'follow_up' and ask only targeted follow-up questions."
    ],
    parameters: Type.Object({
        ...
        Type.Object({
          ...
          choices: Type.Array(
            Type.Object({
              ...
              recommended: Type.Optional(Type.Boolean({ description: "Whether this choice is an advisory recommendation" })),
              recommendationReason: Type.Optional(Type.String({ description: "Short reason shown when this choice is recommended" })),
            }),
            { description: "3–5 domain-specific choices for this question" },
          ),
        }),
        { description: "Initial round: exactly 5 baseline interview questions tailored to the workflow goal. Follow-up round: targeted follow-up questions only." },
      ),
      round: Type.Optional(Type.Union([Type.Literal("initial"), Type.Literal("follow_up")], { description: "Use 'initial' for the first interview round; use 'follow_up' for targeted follow-up questions. The tool adds no automatic questions in either mode." })),
    }),
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      const questions = buildDeepInterviewLiteQuestions(state.workflow.title, params.questions, params.round ?? "initial");
