...
function Test-ComponentSelected([string]$Rel) {
    ...
    foreach ($componentName in $components) {
        $roots = switch ($componentName) {
            "workflow" { @("AGENTS.md", ".pi/.gitignore", ".pi/LOCAL.md", ".pi/WORKFLOW.md", ".pi/GOVERNANCE.md", ".pi/extensions/workflow.ts", ".pi/extensions/workflow", ".pi/dpaa", ".pi/workflows", ".pi/skills", ".pi/personas", ".pi/pyproject.toml", ".pi/schemas/harness-field-log-event.schema.json", ".pi/sbadr", ".pi/setup_corenlp.sh") }
