...
Requirements discovery now focuses on the default workflow `interview → plan` path. The separate feature-interview, feature-planning-room, and requirements-room skills and slash commands have been removed. The `interview` skill now follows a deep-interview-lite style: the initial round confirms top-level component/work-map topology, cites narrow repo evidence first for brownfield work, and targets follow-up questions at the weakest clarity dimension across goal, scope, acceptance criteria, constraints, and existing context. Low-score follow-up wizard rounds pass `round: 'follow_up'` so they show only targeted questions instead of repeating the topology/clarity scaffold. After the wizard completes, the LLM must record goal/scope/acceptance/constraints/context scores (0–100 each) with `workflow_score_interview`; any dimension below 60 blocks the `interview → plan` transition.
