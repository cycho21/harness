...
Requirements discovery now focuses on the default workflow `interview → plan` path. The separate feature-interview, feature-planning-room, and requirements-room skills and slash commands have been removed. The `interview` skill now follows a deep-interview-lite style: it asks the five LLM-provided baseline questions first (scope, motivation, acceptance criteria, affected files/modules, constraints/risks). Runtime does not add a work-map/topology question or a clarity checkpoint automatically. When useful, the LLM may include topology or clarity questions directly in the baseline. Choices may include optional `recommended` and `recommendationReason` fields. Recommendation display is advisory information, and the user's selected answer is the source of truth. Brownfield work should cite narrow repo evidence first, and follow-up questions target the weakest clarity dimension across goal, scope, acceptance criteria, constraints, and existing context. Low-score follow-up wizard rounds pass `round: 'follow_up'` so they show only targeted questions. After the wizard completes, the LLM must record goal/scope/acceptance/constraints/context scores (0–100 each) with `workflow_score_interview`; any dimension below 60 blocks the `interview → plan` transition.
...
EPIC PEV Queue is an optional task queue for large interviews that produce multiple tasks. It keeps the existing workflow phases unchanged and switches only the active task stored in `WorkflowInstance.taskQueue`. The `workflow_task_queue` tool uses `create`, `add`, `activate`, `mark`, and `show` operations to create a queue, select one active task, and record each task as `pending`, `active`, `done`, `blocked`, or `deferred`. The full queue snapshot is written to `.ai/interview/runs/<workflowId>/task-queue.json`, while prompt context shows only the queue title, active task, status counts, next pending task, and bounded scope/acceptance/verification cues for the active task. The implement-phase autonomous task execution policy uses the active task as the goal lock and guides next action, test decision, failure escape, and completion/switch decisions. This policy is a prompt-contract/advisory rule, not a new hard gate. This feature is not unbounded discovery: out-of-scope findings are recorded as pending or deferred tasks instead of being implemented automatically. Existing single-workflow usage and the `commit → push` approval boundary are unchanged.
...
- When `/workflow start <goal>` starts the `interview` phase in a UI session, an interview wizard opens automatically. The LLM still provides the five baseline questions, and runtime displays only those questions. Runtime does not add a work-map/topology question or a clarity checkpoint automatically; scope and affected-file/module coverage come from the baseline answers. When useful, the LLM may include topology or clarity questions directly in the baseline. Choices may include optional `recommended` and `recommendationReason` fields, and the TUI displays a `[추천]` badge plus a reason line for recommended choices. Recommendation display does not auto-select a choice; the user's selected answer is the source of truth. Low-score follow-up wizard rounds pass `round: 'follow_up'` and show only targeted questions. The wizard presents questions one by one, combines choices with free-text input for most questions, allows `unknown/skip` only on optional questions, and blocks required questions until the user selects a choice or enters free text. It shows question progress above the editor and workflow title/current phase/next phase/overall phase progress in the footer. The preview key shows the collected answer summary, and the same summary appears automatically after the final question. The returned wizard result tells the LLM to treat baseline scope and affected-file/module answers as required spec/plan coverage, inspect narrow repo evidence for brownfield follow-ups, and ask focused questions for the weakest clarity dimension before advancing. If UI is unavailable or the wizard is cancelled/fails, the existing chat-based interview continuation remains the fallback.
