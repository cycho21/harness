...
- Guard blocked messages use this section order: `Why blocked`, `Default handling for the LLM`, `Next actions`, optional `Exception path`, and `Caution`. `Exception path` appears only for guards with a skip path. `Default handling for the LLM` states the no-skip-first rule, repair-before-retry for repairable failures, and the restricted conditions for user questions. The workspace guard forbids file edits, workflow-state changes, and evidence simulation, then asks the user to return to the expected directory and branch. The policy-scan guard forbids hiding or silently changing risky files, then asks the agent to present the risk summary and use the interactive policy approval path.
