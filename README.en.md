...
- Guard blocked messages use this section order: `Why blocked`, `Default handling for the LLM`, `Next actions`, optional `Exception path`, and `Caution`. `Exception path` appears only for guards with a skip path. `Default handling for the LLM` states the no-skip-first rule, repair-before-retry for repairable failures, and the restricted conditions for user questions. DPAA/SBADR run before the implementation approval dialog with adaptive strictness derived from the workflow title and plan content: docs/cosmetic/discovery/small work may treat FAIL as advisory and may proceed without a machine-checkable plan, normal feature work keeps FAIL blocking, and API/schema/security/migration/data/deployment work stays strict. The verification layer accepts binary/observable acceptance conditions such as `command exits 0`, `tests pass`, `README updated`, and `no blockers/errors` in addition to numeric metrics. WARN remains advisory and does not block the transition. The workspace guard forbids file edits, workflow-state changes, and evidence simulation, then asks the user to return to the expected directory and branch. The policy-scan guard forbids hiding or silently changing risky files, then asks the agent to present the risk summary and use the interactive policy approval path.
