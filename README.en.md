...
The template also includes supporting reasoning-protocol skills. `trace` narrows workflow/guard/runtime/test anomalies through observation, hypotheses, evidence, rebuttal, and the next discriminating probe before fixes; `verify` turns completion claims into concrete verification evidence; and `cleanup` removes AI slop such as duplication, dead code, and needless abstraction through small behavior-preserving, verified passes. `/workflow trace <observation>` starts the trace protocol with the current workflow context.
